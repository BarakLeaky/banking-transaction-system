  ...
  const result = accounts.map((account) => ({
    ...
    loan: account.loan_id
      ? {
          ...
          outstanding: account.loan_outstanding,
          status: account.loan_status
        }
      : null
  }));
...
router.post("/", (req, res) => {
  ...
  if (!Number.isFinite(deposit) || deposit < 0) {
    return res.status(400).json({
      message: "Initial deposit must be zero or greater"
    });
  }
  ...
  while (
    ...
    accountNumber =
      generateAccountNumber() + Math.floor(Math.random() * 9);
  }
...
router.post("/transfer", (req, res) => {
  ...
  const run = db.transaction(() => {
    ...
    db.prepare(
      ...
      sourceBalance,
      sourceBalance === 0 ? "DORMANT" : "ACTIVE",
      from.id
    );
    ...
    db.prepare(
      `
      INSERT INTO transactions
      (account_id, type, amount, reference_account, description)
      VALUES (?, 'TRANSFER_OUT', ?, ?, 'Transfer sent')
      `
    ).run(
      from.id,
      amount,
...
// Delete a dormant account with zero balance
router.delete("/:id", (req, res) => {
  ...
  if (account.balance !== 0 || account.status !== "DORMANT") {
    return res.status(400).json({
      message: "Only dormant accounts with a zero balance can be deleted"
    });
  }
  ...
  res.json({
    message: "Dormant account deleted"
  });
});

// Create loan account and disburse KES 10,000
router.post("/:id/loan", (req, res) => {
  ...
  if (existing) {
    return res.status(409).json({
      message: "Account already has an active loan"
    });
  }
  ...
  const run = db.transaction(() => {
    db.prepare(
      `
      INSERT INTO loans
      (account_id, principal, outstanding)
      VALUES (?, ?, ?)
      `
    ).run(
      account.id,
    ...
    db.prepare(
      ...
      INSERT INTO transactions
      (account_id, type, amount, description)
      VALUES (?, 'LOAN_DISBURSEMENT', ?, 'KES 10,000 starter loan')
      `
    ).run(
      account.id,
      amount
...
// Get latest loan for an account
